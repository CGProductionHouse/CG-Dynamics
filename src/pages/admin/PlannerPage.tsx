...
function PlannerTaskDrawer({ task, buckets, people, currentProfileId, actorName, canManage, assignmentDisabled, onClose, onReload, onArchived }: {
  ...
  const inputClass = 'w-full rounded-lg border border-white/10 bg-[#111] px-3 py-2 text-sm text-white focus:outline-none focus:ring-1 focus:ring-brand-accent disabled:cursor-not-allowed disabled:opacity-55'
  ...
  return (
    <>
      ...
      <aside className="fixed inset-y-0 right-0 z-50 flex w-full flex-col border-l border-white/10 bg-[#101010] sm:w-[32rem]" aria-label="Task details">
        <div className="flex items-center justify-between border-b border-white/10 px-5 py-4"><div><h2 className="font-bold text-white">Task details</h2>{task.original_plan_name && <p className="text-[10px] text-white/35">Imported from {task.original_plan_name}</p>}</div><button type="button" onClick={requestClose} className="min-h-10 min-w-10 rounded-lg text-xl text-white/60" aria-label="Close task details">&times;</button></div>
        ...
        <div className="border-t border-white/10 px-5 py-4">{saveError && <p className="mb-2 text-xs text-red-300">{saveError}</p>}{saveMessage && <p className="mb-2 text-xs text-brand-teal">{saveMessage}</p>}<div className="flex items-center gap-3"><ActionButton variant="primary" onClick={save} disabled={saving || !title.trim() || (!canManage && !canUpdateStatus) || (canManage && assignmentDisabled)} loading={saving}>Save</ActionButton><button type="button" onClick={requestClose} className="rounded-lg border border-white/10 px-4 py-2 text-sm text-white/60">Close</button>{canManage && (!confirmArchive ? <button type="button" onClick={() => setConfirmArchive(true)} className="ml-auto text-xs text-amber-300/70">Remove from active</button> : <div className="ml-auto flex gap-2 text-xs"><button type="button" onClick={archive} className="text-amber-200">Confirm remove</button><button type="button" onClick={() => setConfirmArchive(false)} className="text-white/45">Cancel</button></div>)}</div></div>
      </aside>
