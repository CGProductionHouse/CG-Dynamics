...
function generateProjectInstructions(clientName: string): string {
  const lines = [
    ...
    '',
    `Work only on ${clientName}. CG Dynamics is permanent truth. For client-dependent work, retrieve current canonical intelligence from CG Dynamics at task time.`,
    '- Never rely on a previously uploaded .md file.',
    '- When an entity/sub-brand/branch is supplied, retrieve that exact scope; never fall back to a sibling or national/group record.',
    '- Write human, specific copy. Captions add to artwork/video rather than repeat it.',
    '- Max 5 hashtags, chosen dynamically for the exact topic, platform, location and search intent.',
    '- Caption/content → fetch voice rules, caption rules, verified facts, guardrails, and the current approved contact/footer policy.',
    '- Image editing → fetch visual/image rules; preserve real people, products, branding, proportions and composition.',
    '- Factual claims → fetch verified facts, provenance and freshness; flag conflicts or gaps instead of guessing.',
    '- SEO/hashtags → choose 3-5 dynamically; do not reuse a saved bank.',
    '- Never hardcode phone numbers, emails, addresses, websites, prices, hours, promotions, stock, daily tasks, live leads, or named current contacts in these Instructions.',
    '- Never fuzzy-match this client or import another client’s facts.',
  ]
