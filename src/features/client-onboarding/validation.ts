...
const ALLOWED_OPTIONAL_EXTENSIONS = new Set([
  'pdf', 'png', 'jpg', 'jpeg', 'zip', 'txt', 'csv',
])
...
export const MAX_ONBOARDING_FILE_BYTES = 50 * 1024 * 1024
...
export function validateLogoCandidate(file: UploadCandidate): string | null {
  ...
  if (file.size > MAX_ONBOARDING_FILE_BYTES) return 'Keep each file under 50 MB.'
  return null
}
...
export function validateOptionalCandidate(file: UploadCandidate): string | null {
  const extension = extensionOf(file.name)
  if (!extension || BLOCKED_EXTENSIONS.has(extension)) return 'This file type is not safe to upload.'
  if (!ALLOWED_OPTIONAL_EXTENSIONS.has(extension)) return 'Use a PDF, image, ZIP, text file, or CSV.'
  if (file.size <= 0) return 'This file appears to be empty.'
  if (file.size > MAX_ONBOARDING_FILE_BYTES) return 'Keep each file under 50 MB.'
  return null
}
