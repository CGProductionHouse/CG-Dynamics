...
// #319 requires an explicit Project context on every client tool call, so the Project must
// bootstrap with resolve_project_context before get_client_context. The original template
// omitted that step, so a Project following it would fail closed on its first client call.
// It also now states the live runtime outranks any uploaded Project Source file, which is
// what caused Projects to read a stale *-CG-DYNAMICS-CLIENT-GUIDE.md instead of Dynamics.
// Bootstrap on the canonical client_id, never client_name: resolve_project_context matches
// names with case-sensitive exact equality, and the ChatGPT Project display name differs from
// clients.name for many clients ("Agri Secure"/"Agri-Secure", "Bohemia Quick Shop"/"...Stop",
// "SupaQuick Bloemfontein"/"Supa Quick BFN"). A name-keyed bootstrap fails closed on those.
const instructions = (id, name) => `Work on the exact CG Dynamics client for this Project (${name}, client_id ${id}).\nFIRST, once per chat: call resolve_project_context with { "context_kind": "client", "client_id": "${id}" } — that exact canonical id, never resolved by name — and reuse the exact context object it returns on every later CG Dynamics call.\nBefore any client-dependent work, call get_client_context with that context and the exact task type. Prefer the live CG Dynamics runtime over any uploaded Project file.\nTreat CLIENT CONTEXT NOT READY as a hard stop for client creative output; surface the missing readiness instead of writing generic copy.\nUse the returned current exact-client guide and task packet. Never borrow from a similar, sibling, group or billing client.\nFor captions and poster copy, use only the returned current caption-approved exact-scope contact/footer packet. Never hardcode or guess contacts, people, addresses, links or mutable facts. If the footer packet is not ready, say so and continue the rest of the task.\nCaptions must add to the supplied creative, follow the client-specific output contract, and avoid generic filler.\nUse no more than 5 dynamically relevant hashtags unless the exact-client guide requires fewer.\nFor image work, preserve real people, products, branding, proportions and composition unless the brief explicitly changes them.\nIf exact-client evidence conflicts or is stale, flag the bounded gap rather than guessing.`
...
const rows = clients.map(([id, name, commit, file]) => {
  ...
  return `    (${q(id)}::uuid, ${q(name)}, ${q(commit)}, ${q(path)}, ${q(b64)}, ${q(Buffer.from(instructions(id, name), 'utf8').toString('base64'))})`
})
