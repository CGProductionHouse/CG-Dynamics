...
Deno.serve(async (req) => {
  ...
  const verifiedAssetIdentities = new Set<string>()
  ...
  while (claimCount < MAX_CHUNKS) {
    if (Date.now() >= invocationDeadline - PAGE_FETCH_RESERVE_MS) break
    ...
    for (const item of items) {
      ...
      try {
        ...
        // Meta does not guarantee media-edge ordering. Exhaust the cursor chain;
        ...
        if (facebookPageId && (!TERMINAL_META_STATES.has(facebookState) || !TERMINAL_META_STATES.has(instagramState))) {
          ...
          if (!verifiedAssetIdentities.has(identity)) {
            pageTokenMap.set(facebookPageId, await fetchMappedPageToken(baseUrl, accessToken, facebookPageId, instagramAccountId, { deadline: invocationDeadline - PAGE_FETCH_RESERVE_MS }))
            verifiedAssetIdentities.add(identity)
          }
        }
