...
drop function if exists public.meta_sync_begin_lane_set(uuid, integer);
create function public.meta_sync_begin_lane_set(p_batch_id uuid, p_lane_count integer default 1)
...
begin
  ...
  update public.meta_sync_batches batch set summary =
    jsonb_set(
      ...
      '{parallel_lane_count}', to_jsonb(greatest(1, least(coalesce(p_lane_count, 1), 6))), true)
...
language sql stable security definer set search_path = '' as $$
  with active as (
    select b.id, greatest(1, least(coalesce((b.summary ->> 'parallel_lane_count')::integer, 1), 6)) lane_count
...
begin
  ...
  if v_duplicate is not null then
    -- Roll back this transaction; review conflicting imported/provider identities
    -- without silently deleting a post or rewriting report curation history.
