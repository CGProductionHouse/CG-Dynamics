...
--
-- Read the exact planner_tasks row referenced by the foreign key. This retains
-- truthful status for linked historical rows even when archived/superseded rows
-- are intentionally absent from active-work projections.
...
begin
  ...
  if v_hour between 7 and 10 then
    ...
    where item.user_id = auth.uid() and item.state = 'open'
      and not exists (
        select 1 from public.planner_tasks linked_task
        where linked_task.id = item.planner_task_id
          and linked_task.status in ('done', 'completed')
      )
