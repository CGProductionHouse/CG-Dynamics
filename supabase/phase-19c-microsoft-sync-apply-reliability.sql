-- ============================================================
-- Phase 19c: Microsoft reconciliation apply reliability repair
--
-- REVIEW IN THE SUPABASE SQL EDITOR BEFORE APPLYING LIVE.
-- Additive and non-destructive. Replaces the existing apply RPC in place,
-- adds an admin-only compatibility preflight, and reloads PostgREST schema.
-- Existing failed/partial runs and per-item audit history remain untouched.
-- ============================================================
...
begin
  if not exists (
    ...
    where table_schema = 'public' and table_name = 'planner_tasks' and column_name = 'assigned_to_name'
  ) or not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'planner_tasks' and column_name = 'helper_names'
  ) or not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'monthly_deliverables' and column_name = 'assigned_to_user_id'
  ) or not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'monthly_deliverables' and column_name = 'assigned_to_name'
  ) or not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'monthly_deliverables' and column_name = 'helper_names'
  ) then
    raise exception 'Microsoft Sync assignment columns are missing; apply the reviewed core/helper migrations first';
  end if;
end;
$$;
...
revoke all on function public.apply_microsoft_sync_item(uuid,text,text,uuid,timestamptz,text,boolean,jsonb,text,text,text,text,boolean,jsonb) from authenticated;
...
comment on function public.microsoft_sync_apply_version() is
  'Admin-only Microsoft reconciliation apply compatibility version. Phase 19c returns 2.';
comment on function public.apply_microsoft_sync_item(uuid,text,text,uuid,timestamptz,text,boolean,jsonb,text,text,text,text,boolean,jsonb) is
  'Admin-only idempotent Microsoft reconciliation apply RPC with assignment persistence.';
