...
test('Meta worker checkpoints safe cursors and resumes both platforms within its deadline', () => {
  ...
  assert.match(metaWorker, /e instanceof RetryableIncompleteError && \(item\.attempts < MAX_PROVIDER_ATTEMPTS \|\| isMetaRateLimitError\(message\)\)/)
  assert.match(metaWorker, /itemStatus = 'queued'/)
  assert.match(metaWorker, /Incomplete pagination exhausted \$\{MAX_PROVIDER_ATTEMPTS\} bounded attempts/)
