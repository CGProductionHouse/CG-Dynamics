...
const META_FENCING = read('../supabase/migrations/20260825190000_meta_sync_fencing_and_idempotency.sql')
...
test('manual and scheduled sync share conservative post reconciliation', () => {
  ...
  assert.match(META_FENCING, /meta_normalize_caption/)
  assert.match(META_FENCING, /jsonb_build_object\('meta_sync'/)
  assert.match(META_FENCING, /report_content_exclusions/)
  assert.match(META_FENCING, /best_poster_post_id/)
  assert.match(META_FENCING, /on conflict \(report_id, platform, meta_post_id\)/)
  assert.match(META_FENCING, /best_video_post_id/)
