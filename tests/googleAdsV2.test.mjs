...
test('equal-window trends require two explicit seven-day windows from canonical rows', () => {
  ...
  assert.equal(weekly?.periodStart, '2026-09-01')
  assert.equal(weekly?.dataThroughDate, '2026-09-14')
  assert.equal(weekly?.spendMicros, 100_000_000)
  assert.equal(weekly?.impressions, 100)
  assert.equal(weekly?.ctr, 20)
  assert.equal(weekly?.conversionRate, 25)
  assert.equal(weekly?.costPerConversionMicros, 20_000_000)
  assert.equal(weekly?.campaigns[0].campaignStatus, 'ENABLED')
