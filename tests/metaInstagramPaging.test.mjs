...
test('worker never assumes ordering or rejects valid unordered media', () => {
  assert.match(worker, /const instagramBoundaryEnabled = false/)
  assert.doesNotMatch(worker, /if \(pageResult\.orderingMalformed\) \{[\s\S]*refusing to traverse unbounded history/)
})
