...
test('staff mobile navigation uses visible destinations plus a complete More menu', () => {
  ...
  assert.match(layout, /MOBILE_PRIMARY_PATHS\.includes\(item\.to\)/)
...
test('mobile bottom bar is role-stable: Hub, Work, Content, Calendar, More', () => {
  ...
  // Manager-only content/work entries never displace primary items.
  assert.doesNotMatch(layout, /MOBILE_PRIMARY_PATHS = \[[^\]]*client-schedule[^\]]*\]/)
  assert.doesNotMatch(layout, /MOBILE_PRIMARY_PATHS = \[[^\]]*team-work[^\]]*\]/)
